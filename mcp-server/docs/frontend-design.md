...
Use `--accent-win` and `--accent-loss` consistently for positive/negative values (point differentials, cap space, win/loss records).
...
**Headlines/Stats**: Use condensed, bold sans-serifs:
- Oswald
- Barlow Condensed
- Anton
- Bebas Neue

**Body/UI**: Clean, legible sans-serif with tabular figures:
- IBM Plex Sans
- Source Sans Pro
- Fira Sans

**Monospace for raw data** (play-by-play logs, box scores):
- JetBrains Mono
- Fira Code

### Type Scale

Headlines should be large and bold. Stats should be scannable. Use tabular figures for columnar numbers:
...
When displaying team-specific content, incorporate team colors as accents:
...
Follow patterns from ESPN, Yahoo Sports, DraftKings, FanDuel:
